...
public class Card implements Serializable
{
    ...
    */
	...
	public void write(DataOutput writer) throws IOException{
		writer.writeChar(this.suit);	
		writer.writeChar(this.rank);
	}
	
	public void write(ObjectOutputStream writer) throws IOException{
	...
	public static Card read(BufferedReader reader) throws EOFException{
		try{
			//String theCardString = reader.readLine();
			...
			//If we can't get the next element, it must be the end of the file.
	...
	public static Card read(DataInput reader) throws IOException{
		char suit = reader.readChar();
		char rank = reader.readChar();
	...
	public static Card read(ObjectInputStream reader) throws IOException, ClassNotFoundException{
		Object newObject = reader.readObject();
		if (newObject instanceof Card){
			return (Card) newObject;
		} else {
			return null;
		}
	}
	...
	public static void main(String[] args) throws FileNotFoundException, IOException{
		...
		Card card2 = new Card(Card.SPADES, Card.JACK);
		PrintWriter pw = new PrintWriter(new File(System.getProperty("user.dir")+"/src/cards/cardfile.txt"));
		...
		BufferedReader bR = new BufferedReader(new FileReader(System.getProperty("user.dir")+"/src/cards/cardfile.txt"));
		Card myCard = Card.read(bR);
		Card mySecondCard = Card.read(bR);		
